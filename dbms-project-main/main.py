...
def bookings(): 
    ...
    if current_user.usertype=="Doctor":
        ...
        query=db.engine.execute(f"SELECT * FROM `patients` WHERE email in (SELECT email from `patients` WHERE email = '{em}')")
        return render_template('booking.html',query=query)
...
def edit(pid):
    ...
    if request.method=="POST":
        ...
        dept=request.form.get('dept')
        number=request.form.get('number')
        db.engine.execute(f"UPDATE `patients` SET `email` = '{email}', `name` = '{name}', `gender` = '{gender}', `slot` = '{slot}', `disease` = '{disease}', `time` = '{time}', `date` = '{date}', `dept` = '{dept}', `number` = '{number}' WHERE `patients`.`pid` in (select `pid` from `patients` where  `pid` = {pid})")
...
@login_required
def delete(pid):
    db.engine.execute(f"DELETE FROM `patients` WHERE `patients`.`pid` in (SELECT pid FROM `patients` where pid = {pid})")
